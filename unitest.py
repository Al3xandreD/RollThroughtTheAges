...
class MyTestCase(unittest.TestCase):
    def test_gestion(self):
        '''On teste si notre gestion du plateau (mode pyramidale avec fonction récursive) et de ses ressources retourne le bon résultat.
        Par exemple on augmente de 4 en partant de rien: on doit avoir 3 bois, 6 pierres, 9 poteries, 12 tissus et 0 fleche '''
        # création variable
        j1 = Player('Joueur 1')
        j2 = Player('Joueur 2')
        ...
        # Tour 1
        j1.gestion(8)  # on augmente les ressources du plateau et on recalcule nos quantité avec notre fonction récursive
        j2.gestion(6)
        j2.gestion(-1) # on imagine que le joueur 2 retire des dés, donc on doit retirer des resources

        self.assertEqual(3, j1.wood.quant)
        self.assertEqual(6, j1.stone.quant)
        self.assertEqual(9, j1.pottery.quant)
        self.assertEqual(4, j1.cloth.quant)
        self.assertEqual(5, j1.arrow.quant)

        self.assertEqual(1, j2.wood.quant)
        self.assertEqual(2, j2.stone.quant)
        self.assertEqual(3, j2.pottery.quant)
        self.assertEqual(4, j2.cloth.quant)
        self.assertEqual(5, j2.arrow.quant)

        #Tour 2
        j1.memoireR=0 # Nouveau tour
        j1.gestion(2)
        self.assertEqual(6, j1.wood.quant)
        self.assertEqual(12, j1.stone.quant)
        self.assertEqual(9, j1.pottery.quant)
        self.assertEqual(4, j1.cloth.quant)
        self.assertEqual(5, j1.arrow.quant)

    def test_effetdeveloppment(self):
        '''Ce test verifie si l'effet d'un développement a bien lieu.
        -Prenons par exemple le development Irrigation il protège de la sécheresse (cas d'un tirage avec deux têtes de mort) qui enleve 2 poins'''

        j3=Player('Joueur 3') # création du joueur
        j3.skull=2 # on imagine un tirage avec deux tetes de morts
        pointav=j3.point[2] # on regarde les points avant le desatre
        j3.desastereffect() # on applique les desatre
        self.assertEqual(j3.point[2],pointav-2) # normalement on devrait perdre deux points

        # Maintenant le joueur achete le developpment
        j3.irrigation.status = True
        pointav2 = j3.point[2]
        j3.desastereffect()  # on applique les desatre
        self.assertEqual(j3.point[2], pointav2)  # normalement on ne devrait pas perdre deux points

    def test_achatcitée(self):
        '''Cette méthode teste si lorsqu'on achete une citée, l'achat est bien effectué'''
        J1 = Player('joueur 1') # Creation du joueur
        self.assertFalse(J1.city_sup1.status) # on regarde si effectivement la citée n'est pas construite
        J1.workers.quant=3 # on donne les ouvrirers nécessaire a la construction
        a,b,c=J1.city_sup1.construire(J1.workers.quant, J1.nb_city) # utilisation de la methode d 'instance "construire" de la classe Construction pour construire la citée
        self.assertTrue(J1.city_sup1.status)# Test si la citée est construite

    def test_modificationdee(self):
        '''Cette méthode regarde si effectivement on ne peut pas modifier la
        valeur d'un dé pour éviter une triche'''
        a=Dee()
        a.throw()
...
####################################   NOTE IMPORTANTE       #########################
# Pour eviter de creer une fonction qui remplit les inputs automatiquement(difficile), nous choisissons de copier le fichier
# et de remplacer les inputs par les valeurs qu'aurait rempli la personne
#Ce processus permet d'avoir un test du code tout aussi efficace et eviter les problemes d'imports
                ...
                # achat monument
                    ...
                    for monument in self.liste_monuments:
                        if monument.test_construction() == True and monument.nm == nom_monument and monument.acces == True:  # test d'aqcuisition
                            ...
                            for joueur in self.liste_joueur:  # on cherche à savoir s'il s'agit de l'acquereur primaire
                ...
                if no == 0:
                    ...
                    for developp in self.liste_developpement:
                        ...
                        if developp.test_construction(S + S2, prix) and developp.nm == nom_dev:  # on acquiert
                            ...
                            self.coins.quant = 0
                            while prix > 0:
                                print("There is still an amount to pay {} resources".format(prix))
                                Lnom2=[]
                                print('')
    ...
    def desastereffect(self):
        ...
        if self.skull == 2:
            ...
            if self.religion.status == True:
                ...
                for joueur in self.liste_joueur:  # tester la religion sur chacun des joueurs et enlever des points
